...
export default function ExtraConsiderations() {
  ...
  const placeholderText = `Example: "I'm designing a video sharing service like Youtube, where users will be able to upload/view/search videos"`
  ...
  const handleShowCopilot = () => {
    ...
    if (systemDesignState.chats.get("extra")!.length < 1) {
      copilotService.getModelFeedback("extra");
    }
  };
  ...
  return (
    <>
      <Container className="description-container">
        <Form onSubmit={handleSubmit}>
          <Form.Group>
            <Form.Label>
              <br />
              <h3 className="header">Extra Considerations & Notes About Your Design</h3>
            </Form.Label>
            <br />
            {/* <Form.Label>
      ...
      <Copilot chatKey="extra" show={showCopilot} handleClose={() => setShowCopilot(false)} />
    </>
  );
}
